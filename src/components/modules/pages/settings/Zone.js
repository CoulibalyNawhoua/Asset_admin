...
function ZonePage(){
    ...
    const submitForm= (e)=>{
        ...
        _formData.append('canal_id',itemData.canal_id);

        setLoadingf(true);
        try {
            axios.post(url.base+'/setting/zone-store',_formData,
            {
                headers:{
                    'Content-Type':'application/json',
                    "Authorization": `Bearer ${localStorage.getItem('_token_')}`
    ...
    const deleteItem = async (id) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    axios.get(url.base+'/setting/zone-delete/?zone_id='+id,
                        {
                            headers:{
                                'Content-Type':'application/json',
                                "Authorization": `Bearer ${localStorage.getItem('_token_')}`                                
    ...
    const submitFormEdit= (e)=>{
        ...
        _formData.append('canal_id',canalId);

        setLoadingf(true);
        try {
            axios.post(url.base+'/zone-update/'+item.id,_formData,
            {
                headers:{
                    'Content-Type':'application/json',
                    "Authorization": `Bearer ${localStorage.getItem('_token_')}`
    ...
    return (
        <>
            <ContentSection ulShownav={"parametres"} navactive={"zone"}>
            <div class="container-fluid">
                    <div class="row">
                        <div class="col-lg-12 col-md-12 col-12">
                            
                            <div class="mb-5">
                                <h3 class="mb-0 ">Liste des zones</h3>
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <form onSubmit={submitForm}>
                            ...
                            <div class="mb-3">
                                    <label class="form-label" for="selectOne">Selectionnez un canal <span
                                    class="text-secondary">(Obligatoire)</span></label>
                                    <select class="form-select" aria-label="Default select example" name="canal_id" onChange={handleChange} value={itemData.canal_id}>
                                        <option value="" selected>----</option>
                                        {canalData?.map((canal,index)=>
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel">Modifier une zone</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close">
                                {/* <span aria-hidden="true">&times;</span> */}
                            </button>
                        </div>
                        <form onSubmit={submitFormEdit}>
                            ...
                            <div class="mb-3">
                                    <label class="form-label" for="selectOne">Selectionnez un canal <span
                                    class="text-secondary">(Obligatoire)</span></label>
                                    <select class="form-select" aria-label="Default select example" name="canal_id" onChange={(e)=>setCanalId(e.target.value)} value={canalId}>
                                        <option value="" selected>----</option>
                                        {canalData?.map((canal,index)=>
                                            <option value={canal.id} >{canal.libelle}</option>
                                        )}
