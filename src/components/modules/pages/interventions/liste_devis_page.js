...
function DevisPageList()
{
    ...
    const validate_devis_intervention=()=>{
        ...
        Swal.fire({
            title: 'ATTENTION !',
            text: 'Êtes-vous sûre valider ce devis.',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                setLoading(true);
                try {
                    axios.get(url.base+'/valider-devis-intervention/'+item?.id,
                        {
                            headers:{
                                'Content-Type':'application/json',
                                "Authorization": `Bearer ${localStorage.getItem('_token_')}`                                
                        ...
                        Swal.fire(
                            "Félicitation",
                            "Le devis a été validé avec succes.",
                            "success"
                        );
                        window.$('#basicModal').modal('hide');
                        fetchList();
          ...
          try {
            axios.post(`${url.base}/rejeter-devis-intervention/${item.id}`,_formData,
           {
              ...
              //  credentials:'include'
               ...
               setLoading(false);
               SwalTopBar({icon:"success",title:"Le devis a été rejeté avec succès !"});  
    ...
    return (
        <>
        <ContentSection ulShownav={"interventions"} navactive={"devis_list"}>
            <div class="container-fluid">
                <div class="row">
                    <div class="col-lg-12 col-md-12 col-12">
                        
                        <div class="mb-5">
                            <h3 class="mb-0 ">Gestion des devis
            ...
            <div class="modal fade" id="basicModal" tabindex="-1"  aria-hidden="true">
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-footer">
                                <button type="button" class={subLoading ? "btn btn-primary disabled":"btn btn-primary"} onClick={()=>Downloadpdf(item?.id)}>Télecharger le devis</button>
                               {item?.status == 0 &&
                                <>
                                    <button type="button" class={loading ? "btn btn-danger disabled" : "btn btn-danger"} onClick={()=>open_modal_commentaire()}>Rejeter le devis</button>
                                    <button type="button" class={loading ? "btn btn-success disabled" : "btn btn-success"} onClick={()=>validate_devis_intervention()}>Valider le devis</button>
                                </>
                               }
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header bg-info-soft ">
                            <h5 class="modal-title" id="exampleModalLabel">Rejeter le devis <b className="text-success">{item?.libelle}</b></h5>
